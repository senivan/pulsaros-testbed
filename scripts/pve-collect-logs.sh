...
collect_host() {
  ...
  collect_cmd "$host" "$ip" pulsaros-netstack-status "sudo systemctl status pulsaros-netstack --no-pager || true"
  collect_cmd "$host" "$ip" pulsaros-netstack-journal "sudo journalctl -b -u pulsaros-netstack --no-pager || true"
  collect_cmd "$host" "$ip" pulsaros-netstack-config "sudo cat /etc/pulsaros/netstack/netstack.conf 2>/dev/null || true"
  collect_cmd "$host" "$ip" pulsaros-netstack-pci-map "sudo cat /etc/pulsaros/netstack/pci-map.json 2>/dev/null || true"
